...
function HitPos( ent )
    local Invalid = Vector(0,0,0)
    local Out = Invalid
    if not IsValid( ent ) then return Invalid end
    local Bone = ent:LookupBone( "ValveBiped.Bip01_Spine" )
    local Backup = 1
    local BackupName = ent:GetBoneName( Backup )
    local ValidBackup = false
    if BackupName then
        ValidBackup = BackupName ~= "__INVALIDBONE__"
    end
    if Bone and Bone >= 0 then 
        local Pos, Ang = ent:GetBonePosition( Bone )
        Out = Pos
    elseif ValidBackup then
        local Bone = Backup
        local Pos, Ang = ent:GetBonePosition( Bone )
        Out = Pos
    else    
    ...
    if not Out then return Invalid end
    -- debugoverlay.Cross( Out, 10, 1, Color( 255,255,255 ), true )
    Out = Out + Vector( 0,0,-20 )
    return Out
end
...
function ENT:EmplFireThink( Fire )
    ...
    if DoReload and self.Reloading == nil then
        ...
        ent:SetPos( self:GetPos() + Forward * 5 + Up * 10 )
        ...
        local Obj = ent:GetPhysicsObject()
        local RandTorque = Vector(100)
        Obj:ApplyForceCenter( ( -Left + Up ) * 100 * Obj:GetMass() )
        Obj:ApplyTorqueCenter( RandTorque * Obj:GetMass() )
...
local function IsAlreadyManning( ent )
    ...
    return ent:IsCurrentSchedule(SCHED_FORCED_GO_RUN) or ent:GetNPCState() == NPC_STATE_SCRIPT
end 
...
local function ThisCanManTurret( self, turret )
    ...
    if HasEnemy then
        ...
        local Shootable = turret:EmplacementCanShoot( HitPos( Enemy ) ) and self:Visible( Enemy ) and TurrVisible
...
function ENT:SearchForPersonToManIt()
    ...
    elseif self.FreeGun and not IsAlreadyManning( self.NpcToManTheTurret ) then
        ...
        for _, CurrentEnt in pairs( NearbyEnts ) do
